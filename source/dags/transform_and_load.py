...
# Fetch environment variables directly (dotenv removed)
CLOUD_STORAGE_BUCKET = os.getenv("GOOGLE_CLOUD_STORAGE_BUCKET")
BIGQUERY_DATASET = os.getenv("GOOGLE_BIGQUERY_DATASET_NAME")
SQL_FILE_PATH = "/home/airflow/gcs/dags/database/create_tables.sql"
...
def transform_and_load(request=None):  # Added request=None for Cloud Functions/Run
    try:
        ...
        for blob in blobs:
            ...
            # Transform the data
            tables_data = {table_name: [] for table_name in table_names}
            ...
            for table_name, rows in tables_data.items():
                if rows:
                    ...
                    if errors:
                        print(f"Errors while inserting into {table_name}: {errors}")
                    else:
